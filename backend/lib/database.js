...
const initSchema = () => {
  db.exec(`
    ...
    -- Episode-Detection junction table (legacy)
    ...
    -- Episode-Frames junction table (for sanitized frames only - Strict ID Linking)
    CREATE TABLE IF NOT EXISTS episode_frames (
      episode_id TEXT NOT NULL,
      detection_id INTEGER NOT NULL,
      PRIMARY KEY (episode_id, detection_id)
    );
    CREATE INDEX IF NOT EXISTS idx_episode_frames_ep ON episode_frames(episode_id);
...
const statements = {
  ...
  // Strict JOIN query - only returns frames explicitly linked to this episode
  ...
  // Episode-Detection link (legacy)
  linkEpisodeDetection: db.prepare(`
    INSERT OR IGNORE INTO episode_detections (episode_id, detection_id) VALUES (?, ?)
  `),

  // Episode-Frames link (strict - for sanitized frames)
  linkEpisodeFrame: db.prepare(`
    INSERT OR IGNORE INTO episode_frames (episode_id, detection_id) VALUES (?, ?)
  `),
...
const database = {
  ...
  episodes: {
    ...
    // Bulk-insert frame links (for sanitized frames)
    linkFrames: (episodeId, detectionIds) => {
      const transaction = db.transaction((ids) => {
        for (const id of ids) {
          statements.linkEpisodeFrame.run(episodeId, id);
        }
      });
      transaction(detectionIds);
    },
  },
...
// Add analysis column to episodes table (run once on startup)
try {
  db.exec('ALTER TABLE episodes ADD COLUMN analysis_json TEXT');
  console.log('[Database] Added analysis_json column to episodes');
} catch (e) {
